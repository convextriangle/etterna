				  ...
				  float fBeatNoOffset)
{
	...
	/* This timer is generally only used for effects tied to the background
	 * music when GameSoundManager is aligning music beats.  Alignment doesn't
	 * handle g_fVisualDelaySeconds. */
...
void
Actor::PostDraw() // reset internal diffuse and glow
{
	m_internalDiffuse = RageColor(1, 1, 1, 1);
	m_internalGlow.a = 0;
}
...
void
Actor::PreDraw() // calculate actor properties
{
	...
	if (m_Effect == no_effect) {
		...
		if (fTimeIntoEffect < m_effect_ramp_to_half) {
			...
			fPercentThroughEffect = SCALE(
			  fTimeIntoEffect, rup_plus_ath, rupath_plus_rdown, 0.5f, 1.0f);
		...
		switch (m_Effect) {
			case diffuse_blink:
				 ...
				 * tempState color? (That would have the same effect with
				 * 1,1,1,1, and allow tweening the diffuse while blinking and
				 * shifting.) */
				for (auto& i : tempState.diffuse) {
					i = bBlinkOn ? m_effectColor1 : m_effectColor2;
					i.a *= fOriginalAlpha; // multiply the alphas so we can fade
										   // even while an effect is playing
				}
				break;
			case diffuse_shift:
				for (auto& i : tempState.diffuse) {
					i = m_effectColor1 * fPercentBetweenColors +
			...
			case diffuse_ramp:
				for (auto& i : tempState.diffuse) {
					i = m_effectColor1 * fPercentThroughEffect +
			...
			case rainbow:
				tempState.diffuse[0] = RageColor(
				  ...
				  RageFastCos(fPercentBetweenColors * 2 * PI +
							  PI * 2.0f / 3.0f) *
					  0.5f +
					0.5f,
				  RageFastCos(fPercentBetweenColors * 2 * PI +
			...
			case wag:
				tempState.rotation +=
				  m_vEffectMagnitude *
				  RageFastSin(fPercentThroughEffect * 2.0f * PI);
				break;
			case spin:
	...
	if (m_internalGlow.a > 0) {
		...
		tempState.glow =
		  tempState.glow + m_internalGlow - m_internalGlow * tempState.glow;
	}
}

void
Actor::BeginDraw() // set the world matrix
{
	DISPLAY->PushMatrix();
...
Actor::UpdateTweening(float fDeltaTime)
{
		   ...
		   && fDeltaTime > 0) // something will change
	{
		...
		if (bBeginning) {
			// Execute the command in this tween (if any). Do this last, and
...
static void
generic_global_timer_update(float new_time,
							float& effect_delta_time,
							float& time_into_effect)
{
...
Actor::UpdateInternal(float delta_time)
{
	switch (m_EffectClock) {
		case CLOCK_TIMER:
			...
			// Wrap the counter, so it doesn't increase indefinitely (causing
			// loss of precision if a screen is left to sit for a day).
...
void
Actor::RemoveWrapperState(size_t i)
{
...
void
Actor::BeginTweening(float time, ITween* pTween)
{
	ASSERT(time >= 0);
...
void
Actor::BeginTweening(float time, TweenType tt)
{
	ASSERT(time >= 0);
...
void
Actor::SetEffectPeriod(float time)
{
	ASSERT(time > 0);
...
bool
Actor::SetEffectTiming(float ramp_toh,
					   float at_half,
					   float ramp_tof,
					   float at_full,
					   float at_zero,
...
void
Actor::ResetEffectTimeIfDifferent(Effect new_effect)
{
...
void
Actor::SetEffectDiffuseBlink(float fEffectPeriodSeconds,
							 const RageColor& c1,
							 const RageColor& c2)
{
...
void
Actor::SetEffectDiffuseShift(float fEffectPeriodSeconds,
							 const RageColor& c1,
							 const RageColor& c2)
{
	ASSERT(fEffectPeriodSeconds > 0);
...
void
Actor::SetEffectDiffuseRamp(float fEffectPeriodSeconds,
							const RageColor& c1,
							const RageColor& c2)
{
	ASSERT(fEffectPeriodSeconds > 0);
...
void
Actor::SetEffectGlowBlink(float fEffectPeriodSeconds,
						  const RageColor& c1,
						  const RageColor& c2)
{
	ASSERT(fEffectPeriodSeconds > 0);
...
void
Actor::SetEffectGlowShift(float fEffectPeriodSeconds,
						  const RageColor& c1,
						  const RageColor& c2)
{
	ASSERT(fEffectPeriodSeconds > 0);
...
void
Actor::SetEffectGlowRamp(float fEffectPeriodSeconds,
						 const RageColor& c1,
						 const RageColor& c2)
{
	ASSERT(fEffectPeriodSeconds > 0);
...
void
Actor::SetEffectRainbow(float fEffectPeriodSeconds)
{
	ASSERT(fEffectPeriodSeconds > 0);
	// todo: account for SSC_FUTURES -aj
...
void
Actor::SetEffectWag(float fPeriod, const RageVector3& vect)
{
	ASSERT(fPeriod > 0);
	// todo: account for SSC_FUTURES -aj
...
void
Actor::SetEffectBounce(float fPeriod, const RageVector3& vect)
{
	ASSERT(fPeriod > 0);
	// todo: account for SSC_FUTURES -aj
	m_Effect = bounce;
...
void
Actor::SetEffectBob(float fPeriod, const RageVector3& vect)
{
	ASSERT(fPeriod > 0);
	// todo: account for SSC_FUTURES -aj
...
void
Actor::SetEffectSpin(const RageVector3& vect)
{
	// todo: account for SSC_FUTURES -aj
	m_Effect = spin;
	m_vEffectMagnitude = vect;
...
void
Actor::SetEffectVibrate(const RageVector3& vect)
{
	// todo: account for SSC_FUTURES -aj
	m_Effect = vibrate;
	m_vEffectMagnitude = vect;
...
void
Actor::SetEffectPulse(float fPeriod, float fMinZoom, float fMaxZoom)
{
	ASSERT(fPeriod > 0);
	// todo: account for SSC_FUTURES -aj
	m_Effect = pulse;
...
void
Actor::SetDiffuseColor(const RageColor& c)
{
...
bool
Actor::TweenState::operator==(const TweenState& other) const
{
#define COMPARE(x)                                                             \
	if ((x) != other.x)                                                        \
		return false;
...
void
Actor::TweenState::MakeWeightedAverage(TweenState& average_out,
				  ...
				  bool warn)
{
	...
	if (GetMessageNameFromCommandName(sCmdName, sMessage)) {
		...
		m_mapNameToCommands[sMessage] =
		  apac; // sCmdName w/o "Message" at the end
	} else {
...
Actor::TweenInfo&
Actor::TweenInfo::operator=(const TweenInfo& rhs)
{
	delete m_pTween;
